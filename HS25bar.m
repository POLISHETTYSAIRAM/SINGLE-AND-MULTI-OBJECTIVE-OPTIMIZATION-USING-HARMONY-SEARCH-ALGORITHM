...
WeightFunction=@(x) fobj25(x);        % Weight Function

nVar=25;             % Number of Deciison Variables
...
for i=1:HMS
    HM(i).Position=randi([VarMin VarMax],VarSize);
    HM(i).Weight=WeightFunction(HM(i).Position);
end

% Sort Harmony Memory
[~, SortOrder]=sort([HM.Weight]);
HM=HM(SortOrder);
...
for it=1:MaxIt
    ...
    for k=1:nNew
        ...
        NEW(k).Position=randi([VarMin VarMax],VarSize);
        for j=1:nVar
            if rand<=HMCR
                % Use Harmony Memory
                i=randi([1 HMS]);
        ...
        % Evaluation
        NEW(k).Weight=WeightFunction(NEW(k).Position);
    ...
    BestWeight(it)=BestSol.Weight ;
    
    % Show Iteration Information
    disp(['Iteration ' num2str(it) ': Best Weight = ' num2str(BestWeight(it))]);
...
figure;
% plot(BestWeight,'LineWidth',2);
semilogy(BestWeight,'LineWidth',2);
xlabel('Iteration');
ylabel('Best Weight');
grid on;
